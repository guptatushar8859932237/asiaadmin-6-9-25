...
export default function RoadTransporter() {
  ...
  const getdata = async (page = 1, search = "") => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}getTransporters`,
        payload
      );
  ...
  const handleAddSupplier = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_BASE_URL}addTransporter`, data)
      .then((res) => {
        toast.success(res.data.message || "Add TransporteR successfully!");
  ...
  const handledelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(`${process.env.REACT_APP_BASE_URL}deleteTransporter`, {
            id: id,
          })
          .then((res) => {
...
const formdata={
    ...
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}updateTransporter`,
        formdata
      )
      .then((res) => {
        toast.success(res.data.message);
