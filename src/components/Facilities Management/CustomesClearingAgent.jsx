...
const pageSize = 10;
export default function CustomesClearingAgent() {
  ...
  const getdata = async (page = 1, search = "") => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}getCustomsClearingAgents`,
        payload
      );
  ...
  const handleAddSupplier = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_BASE_URL}addCustomsClearingAgent`, data)
      .then((res) => {
        toast.success(res.data.message || "Customs Clearing successfully!");
