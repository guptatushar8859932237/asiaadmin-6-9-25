...
const pageSize = 10;
export default function FreightForewarder() {
  ...
  const getdata = async (page = 1, search = "") => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}getFreightForwarders`,
        payload
      );
  ...
  const handleAddSupplier = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_BASE_URL}addFreightForwarder`, data)
      .then((res) => {
        toast.success(res.data.message || "Add Freight Forwarder successfully!");
...
const formdata={
    ...
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}updateFreightForwarder`,
        formdata
      )
      .then((res) => {
        toast.success(res.data.message);
