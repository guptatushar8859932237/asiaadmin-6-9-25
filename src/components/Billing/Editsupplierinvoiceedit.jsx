...
export default function Editsupplierinvoiceedit() {
  ...
  const [selected, setSelected] = useState(""); // selected shipment ID
  const [selectedSupplier, setSelectedSupplier] = useState(""); // selected supplier ID

  const [dat, setDat] = useState([]); // Waybill dropdown list
  ...
  const supplierShipmentInvoiceId = editItem?.supplier_shipment_invoice_id || editItem?.supplier_invoice_id;
  const shipmentId = editItem?.shipment_id;
  ...
  useEffect(() => {
    ...
    // Fetch existing supplier invoice data
  ...
  const getwaybills = () => {
    axios
      ...
      .catch((error) => {
        console.error("Error loading waybills dropdown:", error);
      });
  };
  ...
  const apidataget = async (shipmentIdVal, skipEstimate = false, initialInvoiceData = null) => {
    const payload = {
    ...
    try {
      ...
      if (response.data && response.data.shipment) {
        ...
        setSelected(shipmentIdVal);
        setOpenmodal(false);

        if (!skipEstimate) {
          // Fetch quote estimate details to prepopulate tables
          const freightId = response.data.shipment.freight_id || response.data.shipment.id;
          if (freightId) {
            getFreightQuoteEstimateByShipment(freightId);
          }
        }
      }
    } catch (error) {
  ...
  const getFreightQuoteEstimateByShipment = async (freightId) => {
    try {
      const response = await axios.post(
      ...
      if (response.data && response.data.success && response.data.data) {
        ...
        if (invoiceData) {
          ...
          if (invoiceData.shipment_id) {
            apidataget(invoiceData.shipment_id, true, invoiceData);
          }
  ...
  const estimateCalculate = async () => {
    if (!selected) {
      toast.error("Please select a shipment waybill first.");
      return;
    }
    if (!selectedSupplier) {
    ...
    try {
      ...
      const payload = {
        supplier_shipment_invoice_id: supplierShipmentInvoiceId ? parseInt(supplierShipmentInvoiceId) : null,
        supplier_id: parseInt(selectedSupplier),
        shipment_id: parseInt(selected),
      ...
      console.log("[Edit Invoice] addUpdateSupplierShipmentInvoice payload:", payload);
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}addUpdateSupplierShipmentInvoice`,
        payload
      );
      if (response.data && response.data.success === true) {
        toast.success(response.data.message || "Supplier Shipment Invoice saved successfully");
        navigate("/Admin/Supplier_Invoice");
      } else {
        toast.error(response.data.message || "Failed to save Supplier Shipment Invoice");
      }
    } catch (error) {
  ...
  const selectedWaybillObj = dat.find((item) => String(item.shipment_id) === String(selected));
    ...
    <>
      ...
      {openmodal && (
        <div className="custom-modal">
          <div className="custom-modal-content">
            <div className="custom-modal-header">
              <h5 className="bold">Select Shipment Waybill</h5>
        ...
        <div className="container-fluid">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div className="d-flex align-items-center gap-3">
              ...
              <h4 className="freight_hd mb-0">Edit Supplier Shipment Invoice</h4>
            </div>
