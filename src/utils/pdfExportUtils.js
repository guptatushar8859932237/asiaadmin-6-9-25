...
import { injectPdfGlobalStyles, renderPdfQuoteLayout } from "./pdfQuoteLayout";
...
const getFormControlDisplayValue = (el) => {
  if (el.tagName === "SELECT") {
    ...
    if (!value || optionText === "Select" || value === "Select") {
      return "";
    }

    return optionText.replace(/\s+/g, " ").trim() || value;
  }
...
export const getCellPlainText = (cell) => {
  ...
  const rawText = replacement?.textContent ?? cell.innerText ?? cell.textContent;
...
export const preparePdfCloneForExport = (root) => {
...
export const exportEstimatePdf = async (element, filename = "shipping-estimate.pdf") => {
  ...
  const restoreClone = preparePdfCloneForExport(captureTarget);
